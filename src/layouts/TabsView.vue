  ...
  <admin-layout>
    ...
    <a-tabs v-if="multiPage"
      ...
      <a-tab-pane :key="page.fullPath"
        ...
        <span slot="tab"
              :pagekey="page.fullPath">{{$t(getPageTitleName(page.fullPath))}}</span>
      </a-tab-pane>
    </a-tabs>
...
export default {
  ...
  methods: {
    getPageTitleName (fullPath) {
      if (/\//.test(fullPath)) {
        return fullPath.replace(/^\//, '').replace(/\/$/, '').replace('/', '.') + '.name'
      } else {
        return fullPath
      }
